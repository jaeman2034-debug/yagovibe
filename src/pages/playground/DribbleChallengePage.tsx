...
import { createDribbleChallengeSubmission } from "@/services/challengeService";
import {
  CHALLENGE_FRIEND_SCORE_QUERY_LIMIT,
...
/**
 * PR-10B/PR-10A: 드리블 비동기 챌린지 — PK와 동일 trust 경계(토이 시그널).
 * XP는 Callable이 역대 최고 갱신·UTC 일일 한도 내에서만 지급합니다.
 */
export default function DribbleChallengePage() {
  ...
  const { myBest, friendRows, scoresLoading, scoresError } = useChallengeBestScores(
    DRIBBLE_CHALLENGE_TEMPLATE_ID,
    uid,
    isAnon,
    peerUids,
  ...
  const runDemoRound = () => {
    const conesCleared = Math.floor(Math.random() * 9);
    const durationMs = 5000 + Math.floor(Math.random() * 18000);
    const score = Math.max(0, Math.min(1000, conesCleared * 110 + Math.floor(Math.random() * 35)));
    setPendingMeta({ score, conesCleared, durationMs });
    toast.message(`연습 결과: 콘 ${conesCleared}개 클리어 · ${(durationMs / 1000).toFixed(1)}초 · 점수 ${score}`);
  };
  ...
  const submitPending = async () => {
    ...
    setSubmitting(true);
    try {
      const submissionId = await createDribbleChallengeSubmission(
        uid,
        pendingMeta.score,
        {
          conesCleared: pendingMeta.conesCleared,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-violet-950 to-slate-900 text-slate-50">
      <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-lg items-center justify-between gap-3 px-4 py-3">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-violet-200/90">드리블</span>
      ...
      <main className="mx-auto max-w-lg space-y-6 px-4 pb-16 pt-6">
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <h1 className="text-lg font-bold text-white">비동기 드리블</h1>
          <p className="mt-2 text-sm leading-relaxed text-slate-300">
            장애물 코스를 가볍게 시뮬레이션하고 점수만 남겨요. 점수는{" "}
        ...
        <section className="rounded-2xl border border-violet-400/25 bg-violet-950/30 p-5">
          <h2 className="text-xs font-bold uppercase tracking-wide text-violet-200">한 판 돌리기 (데모)</h2>
          <p className="mt-2 text-sm text-slate-300">콘 클리어 수와 시간이 이 기기에서 임의로 정해집니다.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={runDemoRound}
              disabled={!uid || Boolean(isAnon)}
              className="rounded-xl bg-violet-500 px-4 py-2.5 text-sm font-bold text-white shadow-md shadow-violet-900/30 hover:bg-violet-400 disabled:opacity-40"
            >
              연습 라운드 시뮬레이션
            </button>
            {pendingMeta ? (
          ...
          {pendingMeta ? (
            <p className="mt-3 font-mono text-xs text-slate-400">
              콘 {pendingMeta.conesCleared}개 · {(pendingMeta.durationMs / 1000).toFixed(1)}초
            </p>
          ) : null}
        </section>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          ...
          {peerUids.length > CHALLENGE_FRIEND_SCORE_QUERY_LIMIT ? (
            <p className="mt-2 text-[10px] leading-snug text-slate-500">
              수락 친구 중{" "}
              <span className="font-semibold text-slate-400">최대 {CHALLENGE_FRIEND_SCORE_QUERY_LIMIT}명</span>만
              점수에 반영됩니다. 전체 랭킹이 아닙니다.
            </p>
          ) : null}
          {scoresError ? (
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-200">
              ...
              {friendRows.length === 0 ? (
                ...
                <p className="text-slate-400">친구들의 기록이 아직 없어요.</p>
              ) : topFriend?.best != null ? (
                <p>
                  친구 중 최고:{" "}
        ...
        <p className="text-center text-[11px] text-slate-600">
          템플릿 시드: <code className="text-slate-500">npm run seed:pr10b-template:production</code>
        </p>

        <p className="text-center text-[11px] text-slate-600">
          PK 챌린지는{" "}
          <Link to="/playground/pk" className="text-violet-400 underline-offset-2 hover:underline">
            여기
          </Link>
        </p>
      </main>

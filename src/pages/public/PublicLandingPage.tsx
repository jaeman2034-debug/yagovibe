  ...
  useEffect(() => {
    const fetchPublicRecords = async () => {
      try {
        ...
        // 🔥 모든 팀의 블로그 포스트 조회 (최신순)
        const allPosts: PublicPost[] = [];
        const teamIds = new Set<string>();

        // 모든 팀 조회 (최대 50개로 제한)
        const teamsRef = collection(db, "teams");
        const teamsQuery = query(teamsRef, limit(50));
        ...
        // SEO 메타 태그 (중립)
        const seoTitle = "지역 팀 활동 기록";
        const seoDescription = "지역에서 이어진 팀 활동을 날짜가 남는 기록으로 정리했습니다.";
        ...
        updateMetaTag("og:url", `${window.location.origin}/public`, true);

...
import { FederationOperationsCenterPanel } from "@/components/federation/FederationOperationsCenterPanel";
import {
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
                ...
                <div className="rounded-xl border bg-white p-4">
                  ...
                  {leagueDetailTab === "match" && (
                    <div className="space-y-3">
                      ...
                      <h3 className="font-semibold text-gray-900">경기 관리 · 읽기 전용</h3>
                      {currentLeagueMatches.length === 0 ? (
                        <p className="text-sm text-gray-600">등록된 경기가 없습니다.</p>
                      ) : (
                        <div className={currentLeague?.mode === "tournament" ? "flex items-start gap-4 overflow-x-auto" : "space-y-2"}>
                          {(currentLeague?.mode === "tournament"
                            ? tournamentRounds
                            : [{ order: 0, roundName: "", matches: currentLeagueMatches }]
                          ).map((round) => (
                            <div key={round.order} className={currentLeague?.mode === "tournament" ? "min-w-72 rounded-lg border p-3" : "space-y-2"}>
                              {round.roundName && <div className="mb-2 font-semibold">[{round.roundName}]</div>}
                              {round.matches.map((match) => (
                                <div key={match.id} className="rounded-lg border p-3 text-sm">
                                  <div className="font-medium">{match.homeTeam || "TBD"} vs {match.awayTeam || "TBD"}</div>
        ...
        {activeTab === "ops-center" && (
          <FederationOperationsCenterPanel federationSlug={federationSlug} />
        )}

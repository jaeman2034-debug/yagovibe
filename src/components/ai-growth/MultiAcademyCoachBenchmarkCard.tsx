...
type Props = {
  benchmark: MultiAcademyCoachBenchmarkResult | null;
  loading?: boolean;
  className?: string;
};
...
export function MultiAcademyCoachBenchmarkCard({
  benchmark,
  loading = false,
  className,
}: Props) {
  if (loading) {
    return (
      <div
        ...
        data-testid="multi-academy-coach-benchmark-loading"
      >
        <Loader2 className="h-4 w-4 animate-spin" aria-hidden />
        Coach Benchmark 집계 중…
      </div>
    );
  }
  ...
  return (
    <section
      ...
      data-testid="multi-academy-coach-benchmark-card"
      data-empty={isEmpty ? "true" : "false"}
      aria-label="Multi Academy Coach Benchmark"
    >
      <h3 className="text-sm font-black text-emerald-950">코치 벤치마크</h3>
      {benchmark.subline ? (
      ...
      {academies.length > 0 ? (
        <div className="mt-3 space-y-2" data-testid="multi-academy-coach-benchmark-list">
          {academies.map((academy) => (
            <div
              key={academy.teamId}
              className="rounded-xl border border-emerald-200 bg-white/90 px-3 py-2"
              data-testid={`multi-academy-coach-benchmark-row-${academy.teamId}`}
            >
              <p className="text-sm font-bold text-emerald-950">{academy.teamName}</p>
              <p className="mt-0.5 text-[11px] text-emerald-800">
                코치 {academy.coachCount}명 · 세션 {academy.activeSessionCount}회 · 출석 기록{" "}
                {formatPct(academy.attendanceRecordingRate)} · 성장{" "}
                {formatSignedDelta(academy.growthContributionRate)} · 위험 개선{" "}
                {formatPct(academy.atRiskImprovementRate)}
              </p>
            </div>
          ))}
        </div>

/**
 * CV-1 I11-2 — avatarDraft Coach Validation (Validate / Reject)
 */
import { CheckCircle2, Loader2, ShieldCheck, XCircle } from "lucide-react";
import { useState } from "react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { callReviewAvatarDraft, parseAvatarDraftReviewError } from "@/lib/academy/academyCvCallables";
import type {
  AvatarDraftSnapshotDto,
  AvatarDraftValidationDecisionDto,
...
type Props = {
  ...
  mediaId: string;
  linkId: string;
  draft: AvatarDraftSnapshotDto;
  onReviewed?: () => void;
};

function validationLabel(status: AvatarDraftValidationStatusDto): string {
  switch (status) {
    case "validated":
      return "Validated";
    case "rejected":
...
export function CvAvatarDraftValidationPanel({
  teamId,
  mediaId,
  linkId,
  draft,
  ...
  const [validatedBy, setValidatedBy] = useState(draft.validatedBy);
  const [validatedAt, setValidatedAt] = useState(draft.validatedAt);
  const [coachNote, setCoachNote] = useState(draft.coachNote ?? "");
  const [noteDraft, setNoteDraft] = useState("");

  const isPending = validationStatus === "pending";

  async function submit(decision: AvatarDraftValidationDecisionDto) {
    setSubmitting(true);
    setError(null);
    try {
      const result = await callReviewAvatarDraft({
        teamId,
        mediaId,
        linkId,
      ...
      const parsed = parseAvatarDraftReviewError(e);
      if (parsed.message.includes("already-reviewed")) {
        setError("이미 검토된 Avatar draft입니다.");
      } else {
  ...
  return (
    <div
      className="mt-2 rounded-md border border-fuchsia-300 bg-fuchsia-50/60 px-2 py-1.5"
      data-testid="cv-avatar-draft-validation-panel"
    >
      <p className="flex items-center gap-1 text-[10px] font-semibold uppercase text-fuchsia-900">
        <ShieldCheck className="h-3 w-3" />
        Coach Validation (I11-2)
      </p>
      ...
      {isPending ? (
        <>
          <textarea
            className="mt-2 w-full rounded border border-fuchsia-200 bg-white px-2 py-1 text-[11px]"
            rows={2}

/**
 * CV-1 I10-2 — ovrDraft Coach Validation (Validate / Reject)
 */
import { CheckCircle2, Loader2, ShieldCheck, XCircle } from "lucide-react";
import { useState } from "react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { callReviewOvrDraft, parseOvrDraftReviewError } from "@/lib/academy/academyCvCallables";
import type {
  OvrDraftSnapshotDto,
  OvrDraftValidationDecisionDto,
...
type Props = {
  ...
  mediaId: string;
  linkId: string;
  draft: OvrDraftSnapshotDto;
  onReviewed?: () => void;
};

function validationLabel(status: OvrDraftValidationStatusDto): string {
  switch (status) {
    case "validated":
      return "Validated";
    case "rejected":
...
export function CvOvrDraftValidationPanel({
  teamId,
  mediaId,
  linkId,
  draft,
  ...
  async function submit(decision: OvrDraftValidationDecisionDto) {
    setSubmitting(true);
    setError(null);
    try {
      const result = await callReviewOvrDraft({
        teamId,
        mediaId,
        linkId,
      ...
      const parsed = parseOvrDraftReviewError(e);
      if (parsed.message.includes("already-reviewed")) {
        setError("이미 검토된 OVR draft입니다.");
      } else {
  ...
  return (
    <div
      className="mt-2 rounded-md border border-rose-300 bg-rose-50/60 px-2 py-1.5"
      data-testid="cv-ovr-draft-validation-panel"
    >
      <p className="flex items-center gap-1 text-[10px] font-semibold uppercase text-rose-900">
        <ShieldCheck className="h-3 w-3" />
        Coach Validation (I10-2)
      </p>
      ...
      {isPending ? (
        <>
          <textarea
            className="mt-2 w-full rounded border border-rose-200 bg-white px-2 py-1 text-[11px]"
            rows={2}

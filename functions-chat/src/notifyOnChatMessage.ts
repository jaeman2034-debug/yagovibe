...
async function collectPushTokensForUser(
  db: Firestore,
  uid: string
): Promise<string[]> {
  ...
  try {
    ...
    devicesSnap.forEach((doc) => {
      ...
      if (enabled && t && typeof t === "string") tokens.add(t);
    });
  } catch (e: any) {
    logger.warn("⚠️ [notifyOnChatMessage] devices 조회 실패", { uid, err: e?.message });
  }
  ...
  try {
    ...
    logger.warn("⚠️ [notifyOnChatMessage] fcmTokens 조회 실패", { uid, err: e?.message });
  }
  ...
  try {
    ...
    if (Array.isArray(legacy)) {
      legacy.forEach((t: unknown) => {
        if (t && typeof t === "string") tokens.add(t);
      });
    }
  } catch (e: any) {
    logger.warn("⚠️ [notifyOnChatMessage] users 문서 토큰 조회 실패", { uid, err: e?.message });
  }
...
async function cleanupInvalidPushToken(
  db: Firestore,
  uid: string,
  token: string
): Promise<void> {
  ...
  try {
    ...
    await batch.commit();
    logger.info("🧹 [notifyOnChatMessage] devices 무효 토큰 비활성화", {
      uid,
      count: snap.size,
    });
  } catch (e: any) {
    logger.warn("⚠️ [notifyOnChatMessage] devices 토큰 정리 실패", {
      uid,
      err: e?.message,
    });
  }
...
export const notifyOnChatMessage = onDocumentCreated(
  ...
  async (event) => {
    ...
    try {
      ...
      if (!msg) {
        logger.warn("⚠️ [notifyOnChatMessage] 메시지 데이터 없음", { messageId, chatId });
        return;
      }

      if (msg._mergedFromChatId) {
        logger.info("⏭️ [notifyOnChatMessage] 마이그레이션 복사본 → 알림 스킵", {
          chatId,
          messageId,
        });
        return;
      ...
      if (!senderId) {
        logger.warn("⚠️ [notifyOnChatMessage] senderId 없음 — 스킵", { chatId, messageId });
        return;
      }
      ...
      if (!hasRenderableBody) {
        logger.info("⏭️ [notifyOnChatMessage] 표시할 내용 없음 — 스킵", {
          chatId,
          messageId,
          type: msg.type,
        });
      ...
      if (
        ...
        logger.info("⏭️ [notifyOnChatMessage] 시스템 메시지 → 알림 스킵");
        return;
      }
      ...
      if (!chatSnap.exists) {
        logger.warn("⚠️ [notifyOnChatMessage] 채팅방 없음", { chatId, messageId });
        return;
      }
      ...
      const usersArr: string[] =
        ...
        (chat.users as string[]) ||
        [];
      const receiverId = usersArr.find((u) => u && u !== senderId);

      if (!receiverId) {
        logger.warn("⚠️ [notifyOnChatMessage] 수신자 없음", { chatId, messageId });
        return;
      }

      if (receiverId === senderId) {
        logger.warn("⏭️ [notifyOnChatMessage] 수신자=발신자 — 스킵", { chatId, messageId });
        return;
      }
      ...
      if (unread <= 0) {
        logger.info("⏭️ [notifyOnChatMessage] 미읽음 없음 → 알림 스킵", {
          chatId,
          messageId,
          receiverId,
        });
      ...
      if (!chatNotificationsEnabled) {
        logger.info(`⏭️ [notifyOnChatMessage] 사용자 ${receiverId} 알림 꺼짐`);
        return;
      }

      const tokens = await collectPushTokensForUser(db, receiverId);
      if (tokens.length === 0) {
        logger.info(`⚠️ [notifyOnChatMessage] 사용자 ${receiverId}의 FCM 토큰 없음 (정상 — 미등록)`, {
          chatId,
          messageId,
        });
        return;
      ...
      logger.info(`📱 [notifyOnChatMessage] 푸시 발송 시도`, {
        chatId,
        messageId,
        receiverId,
      ...
      let messagePreview =
        msg.type === "image" ? "📷 사진을 보냈습니다" : String(text || "");
      ...
      try {
        ...
        logger.error(`❌ [notifyOnChatMessage] FCM 발송 실패`, {
          chatId,
          messageId,
        ...
        if (sendError?.code === "messaging/invalid-registration-token") {
          logger.warn(`🧹 [notifyOnChatMessage] 무효 토큰 일괄 정리 (devices+fcmTokens)`, {
            receiverId,
            messageId,
          });
      ...
      logger.info(`✅ [notifyOnChatMessage] 알림 발송 완료`, {
        chatId,
        messageId,
        receiverId,
      ...
      if (response.failureCount > 0) {
        ...
        response.responses.forEach((resp, idx) => {
          if (!resp.success) {
            ...
            if (
              ...
              const t = tokens[idx];
              if (t) failedTokens.add(t);
              logger.info(`🗑️ [notifyOnChatMessage] 무효 토큰 정리 대상`, {
                chatId,
                messageId,
        ...
        if (failedTokens.size > 0) {
          logger.info(`🧹 [notifyOnChatMessage] 무효 토큰 정리 완료`, {
            chatId,
            messageId,
      ...
      logger.error("❌ [notifyOnChatMessage] 상위 처리 실패", {
        chatId,
        messageId,
        error: error?.message,
        stack: error?.stack,

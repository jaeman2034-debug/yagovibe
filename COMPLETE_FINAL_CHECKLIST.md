...
teams/{teamId}/fees/{YYYY-MM}
  month, baseAmount, status: open|closed, createdAt
```
...
teams/{teamId}/fees/{YYYY-MM}/payments/{memberId}
  ...
  paidAt, paidBy, method: manual|auto|import
  amount, dueAmount, paidAmount, carryOverAmount
```

---

## 📅 3) 스케줄 + Lazy-create 병행 ✅
...
**처리:**
- `enableNewFeeSystem=true` 팀 대상
- `fees/{YYYY-MM}` 생성
- `payments/{memberId}` 기본 `unpaid` 생성
...
**처리:**
- 화면 진입 시 `ensureMonthInitialized(teamId, month)` 실행
- 해당 월 doc 없으면 생성
...
1. ✅ `functions/src/feePaymentFinal.ts` - 수동 완납
2. ✅ `functions/src/feeSystemCoreFinal.ts` - 핵심 로직
3. ✅ `functions/src/createMonthlyFeesJobFinal.ts` - 스케줄 + Lazy-create

### 마이그레이션

4. ✅ `scripts/backfillFeesFinal.ts` - 마이그레이션 스크립트

### 프론트

5. ✅ `src/utils/calculateUnpaidMonthsFinal.ts` - 미납 계산식

# 🗺️ Google Maps API 키 리퍼러 제한 오류 해결 가이드

## 📊 현재 확인된 정보

`.env.production` 파일에서 확인:
```
VITE_GOOGLE_MAPS_API_KEY=AIzaSyCJ0ahD8gJGDIGM3GWOob3tsaVS4D93WCw
```

**문제**: `www.yagovibe.com/voice-map`에서 `RefererNotAllowedMapError` 발생

## 🎯 해결 방법

### Step 1: Google Cloud Console에서 Maps API 키 찾기

1. **Google Cloud Console 접속**
   ```
   https://console.cloud.google.com/apis/credentials?project=yago-vibe-spt
   ```

2. **API 키 목록 확인**
   - "API 키 (API Keys)" 섹션에서 확인
   - 현재 보이는 키:
     - **Browser key**: `AlzaSyCNxoZLo5si4EvLqw1eLIUgjf3MzMHyxDY` (Firebase용)

3. **Maps API 키 찾기**
   - **방법 1**: 각 키의 "키 표시" 버튼을 클릭하여 키 값이 `AIzaSyCJ0ahD8gJGDIGM3GWOob3tsaVS4D93WCw`인지 확인
   - **방법 2**: 키 이름이 "Maps API key" 또는 비슷한 이름인 키 찾기
   - **방법 3**: 모든 키를 하나씩 열어서 키 값 비교

### Step 2: Maps API 키를 찾지 못한 경우

#### 경우 1: Browser key와 다른 키일 때

만약 Maps API 키를 별도로 찾을 수 없다면:

1. **"+ 사용자 인증 정보 만들기" → "API 키" 선택**
   - 새 API 키 생성

2. **키 이름 변경**
   - "Maps API key"로 이름 변경 (편집)

3. **.env.production 파일 업데이트**
   - 생성된 새 키로 `VITE_GOOGLE_MAPS_API_KEY` 값 변경

#### 경우 2: Browser key와 동일한 키를 사용하는 경우

만약 Browser key (`AlzaSyCNxoZLo5si4EvLqw1eLIUgjf3MzMHyxDY`)를 Maps API에도 사용한다면:

⚠️ **주의**: Browser key의 값이 `AlzaSyCNxoZLo5si4EvLqw1eLIUgjf3MzMHyxDY`인데,
`.env.production`의 키는 `AIzaSyCJ0ahD8gJGDIGM3GWOob3tsaVS4D93WCw`입니다.
**두 키가 다릅니다!**

따라서 Maps API 키 (`AIzaSyCJ0ahD8gJGDIGM3GWOob3tsaVS4D93WCw`)를 별도로 찾아야 합니다.

### Step 3: HTTP 리퍼러 제한 설정

1. **Maps API 키 편집**
   - 키 이름 클릭 또는 "편집" 아이콘 클릭

2. **애플리케이션 제한사항 설정**
   - "애플리케이션 제한사항" 섹션에서:
     - **"HTTP 리퍼러(웹사이트)"** 선택

3. **웹사이트 제한사항에 도메인 추가**
   
   다음 항목들을 **한 줄씩** 추가:
   ```
   https://www.yagovibe.com/*
   https://yagovibe.com/*
   https://yago-vibe-spt.web.app/*
   https://yago-vibe-spt.firebaseapp.com/*
   http://localhost:5173/*
   http://127.0.0.1:5173/*
   ```

   **⚠️ 중요 포인트**:
   - 각 항목을 한 줄씩 입력
   - `/*` 와일드카드 **반드시** 포함
   - `https://` 프로토콜 명시
   - `www.yagovibe.com`과 `yagovibe.com` **둘 다** 추가

4. **저장** 버튼 클릭

### Step 4: 설정 전파 대기

- **5-10분** 대기 (Google 서버에 설정이 전파되는 시간)
- 때로는 **최대 15분**까지 소요될 수 있음

### Step 5: 브라우저 캐시 삭제 및 테스트

1. **모바일 브라우저 캐시 삭제**
   - Chrome: 설정 → 개인정보 보호 및 보안 → 인터넷 사용 기록 삭제
   - Safari: 설정 → Safari → 인터넷 사용 기록 및 웹사이트 데이터 지우기

2. **테스트**
   ```
   https://www.yagovibe.com/voice-map
   ```

## 🔍 Maps API 키가 보이지 않는 경우

### 가능한 원인

1. **다른 프로젝트에 있을 수 있음**
   - 상단 프로젝트 선택 드롭다운에서 다른 프로젝트 확인

2. **삭제되었을 수 있음**
   - "삭제된 사용자 인증 정보 복원" 버튼 클릭하여 확인

3. **키 이름이 다를 수 있음**
   - 모든 API 키를 하나씩 열어서 키 값 확인

4. **Browser key를 재사용하는 경우**
   - Browser key의 HTTP 리퍼러 제한에 Maps API 도메인도 추가

## 📝 체크리스트

- [ ] Google Cloud Console 접속
- [ ] API 키 목록에서 `AIzaSyCJ0ahD8gJGDIGM3GWOob3tsaVS4D93WCw` 찾기
- [ ] 키를 찾지 못하면:
  - [ ] 다른 프로젝트 확인
  - [ ] 삭제된 키 복원 시도
  - [ ] 새 API 키 생성 고려
- [ ] 찾은 키 편집
- [ ] HTTP 리퍼러 제한 활성화
- [ ] `https://www.yagovibe.com/*` 추가
- [ ] `https://yagovibe.com/*` 추가
- [ ] 다른 도메인들 추가
- [ ] 저장 버튼 클릭
- [ ] 5-10분 대기
- [ ] 브라우저 캐시 삭제
- [ ] `https://www.yagovibe.com/voice-map` 테스트

## 💡 중요 참고사항

- **Maps API 키**: `AIzaSyCJ0ahD8gJGDIGM3GWOob3tsaVS4D93WCw`
- **Browser key**: `AlzaSyCNxoZLo5si4EvLqw1eLIUgjf3MzMHyxDY` (다른 키!)
- **프로젝트**: `yago-vibe-spt`
- **Google Cloud Console**: https://console.cloud.google.com/apis/credentials?project=yago-vibe-spt

---

**이 가이드를 따라 Maps API 키를 찾고 HTTP 리퍼러 제한을 설정하면 오류가 해결됩니다!** 🎉


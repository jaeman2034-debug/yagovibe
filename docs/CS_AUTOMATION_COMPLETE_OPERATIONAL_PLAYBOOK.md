...
**목표**: CS 자동화 99% 안정, 분쟁 97% 감축, 매출 4.0% 확보  
**배포 상태**: ✅ 프로덕션 준비 완료
...
**조건**: 수익률 < 4.0% (목표 대비 10% 이상 하락)  
**처리**: 광고 비율 감소 (10% → 8%, 최소 5%까지)

**복원 조건**: 수익률 ≥ 4.0% 달성 시 점진적 복원

**실행 주기**: 매일 00:00 (Asia/Seoul)

```typescript
const PROFIT_MARGIN_TARGET = 4.0; // 4.0%
...
**조건**: 최근 1시간 내 분쟁률 ≥ 0.18% (0.2% → 0.18%로 강화)  
...
**조건**: 최근 5분 내 봇 응답 15초 초과 분쟁 존재 (18초 → 15초로 강화)  
...
- [ ] 매출 보호 루프 함수 코드 검증
- [ ] 분쟁률 경고 로직 검증 (0.18% 기준)
- [ ] SLA 초과 알람 로직 검증 (15초 기준)
- [ ] 카테고리 급등 검수 로직 검증
- [ ] 고위험 카테고리 검수 로직 검증
- [ ] 수익률 모니터링 로직 검증 (4.0% 목표)
...
- [ ] 운영 대시보드 트리거 동작 확인
- [ ] 매출 보호 루프 동작 확인
- [ ] 분쟁률 경고 동작 확인 (0.18% 기준)
- [ ] SLA 초과 알람 동작 확인 (15초 기준)
- [ ] 카테고리 급등 검수 동작 확인
- [ ] 고위험 카테고리 검수 동작 확인
- [ ] 수익률 모니터링 동작 확인 (4.0% 목표)
- [ ] 마진 하락 광고 비율 자동 조정 동작 확인

---

**CS 자동화 완전 운영 플레이북 완성**

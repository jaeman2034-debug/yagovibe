...
1. `src/pages/me/resolvePersona.ts`
   - 순수 함수로 완전 분리
   - 외부 의존성 제거 (Firestore, React, useQuery, try/catch 없음)
   - Persona 타입 직접 정의

2. `src/pages/me/MePage.tsx`
   - `useMePersona` 훅 제거
   - 데이터 훅 직접 호출 (`useMyProfile`, `useMyTeams`, `useMyTournamentApplications`)
   - `resolvePersona` 직접 사용
   - 조건문 제거 (로그인/로딩만)

3. `src/components/me/IdentityHeader.tsx`
   - Persona 타입 import 경로 변경 (`resolvePersona.ts`)
...
### resolvePersona.ts (순수 함수로 완전 분리)

**변경 전:**
```typescript
import type { Persona } from "@/hooks/useMePersona";  // ❌ 외부 의존성
```

**변경 후:**
```typescript
export type Persona = "ANON" | "P0" | "P1" | "P2" | "P3" | "P4";  // ✅ 직접 정의

export function resolvePersona({ ... }: ResolvePersonaInput): Persona {
  // ✅ 순수 함수 (Firestore ❌ / React ❌ / useQuery ❌ / try/catch ❌)

...
- **Fallback 비율**: 5% 넘어가면 원인 분석
- **P95 latency**: 1200ms 넘으면 UX 불안
...
5. **완전 막힘** → Ultimate Fallback (최소한 "지도" 검색)
...
### 3. 로깅 시스템 연동
- [ ] Firestore/BigQuery 로그 저장 (선택)
- [ ] Sentry 에러 추적 (선택)
- [ ] Cloud Monitoring 알림 (선택)
...
### 비용 최적화
- **SEARCH 비율 증가**: LLM만 호출 (비용 절감)
- **Places 호출 최소화**: NAVIGATE만 사용

### 성능 최적화
- **타임아웃 튜닝**: P95 latency 기준 조정
- **재시도 정책**: 실패율 기준 조정

### 안정성 강화
- **Fallback 체인**: 모든 경로에서 실행 보장
- **로깅 강화**: 장애 원인 빠른 파악

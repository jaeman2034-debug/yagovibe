/**
 * 팀 운영진 대표 사진 — 클라이언트 Storage 쓰기 없이 Callable + Admin SDK 업로드
 */
...
import { applyTeamCaptainPhotoMetaToTeam } from "./setTeamCaptainPhotoMetaCallable";

const REGION = "asia-northeast3";
const MAX_BYTES = 8 * 1024 * 1024;
const FILE_KEY = "captain";

function storagePath(teamId: string): string {
  return `teamCaptainPhotos/${teamId}/${FILE_KEY}`;
}
...
export const uploadTeamCaptainPhoto = onCall(
  {
    region: REGION,
    maxInstances: 10,
    timeoutSeconds: 120,
  ...
  async (request) => {
    ...
    if (clear) {
      try {
        ...
        logger.warn("[uploadTeamCaptainPhoto] storage delete", { teamId, err: String(e) });
      }
      await applyTeamCaptainPhotoMetaToTeam(firestore, teamId, { clear: true });
      logger.info("[uploadTeamCaptainPhoto] cleared", { teamId, uid });
    ...
    await applyTeamCaptainPhotoMetaToTeam(firestore, teamId, { clear: false, captainPhotoUrl: downloadUrl });

    logger.info("[uploadTeamCaptainPhoto] ok", { teamId, uid, bytes: buffer.length });
    return { ok: true as const, captainPhotoUrl: downloadUrl };
  }
);

...
- **변경 사항**:
  ...
  - 게시 버튼 활성화 로직 업데이트 (`canPublish` → `canPublishTournament`)
  - 저장 버튼 비활성화 로직 업데이트 (`adminLoading` → `adminLoadingState`)
  - 게시 권한 체크 로직 업데이트 (`canPublish` → `canPublishTournament`)
...
- ✅ `src/components/association/tournament/TournamentEditDrawer.tsx`
  - `useIsAssociationOwner` Hook 추가
  - `canPublishTournament` 변수 추가 (isOwner || canPublish)
  - 게시 버튼 활성화 로직 업데이트
  - 저장 버튼 비활성화 로직 업데이트
  - 게시 권한 체크 로직 업데이트
- ✅ `firestore.rules`

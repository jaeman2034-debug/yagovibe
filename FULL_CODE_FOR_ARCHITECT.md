...
1. **GET `/api/admin/dashboard/summary?region=seoul&date=2026-02-03`**
   - Response: KPI + 위험 신호

2. **GET `/api/admin/dashboard/health?region=seoul&date=2026-02-03`**
   - Response: 스토리 채움률, 오프라인률, 시드률

3. **GET `/api/admin/dashboard/stories?region=seoul`**
   - Response: 스토리 현황 및 상태별 분류

4. **GET `/api/admin/dashboard/experiments`**
   - Response: AB 실험 모니터 데이터

5. **GET `/api/admin/dashboard/settlements?ownerId=xxx&region=seoul&status=READY`**
   - Response: 정산 관제 데이터

6. **GET `/api/leagues?region=seoul`**
   - Response: 리그 목록

7. **GET `/api/stories?region=seoul`**
   - Response: 스토리 목록
...
### AdminDashboard.tsx
- **Firestore 직접 조회**: `collection(db, "users")`, `collection(db, "auth_logs")`
- **데이터**: 유저 통계, 온보딩 퍼널, SMS 로그
- **백엔드 API 사용**: 없음

### AdminHome.tsx
- **Firestore 직접 조회**: `collection(db, "reports")`, `collection(db, "aiReports")`, `collection(db, "deployHistory")`, `collection(db, "marketProducts")`, `collection(db, "sellerProfiles")`
- **데이터**: AI 리포트, 배포 이력, 사기 위험 상품, 판매자 통계
- **백엔드 API 사용**: 없음

### admin.dashboard.ts (백엔드)
- **구현 완료**: 5개 엔드포인트 모두 구현됨
- **데이터 소스**: Prisma (임시 메모리 스토어)
- **상태**: 사용 가능

#!/usr/bin/env node
// scripts/backfillFees.ts
// 🔄 회비 데이터 마이그레이션 스크립트 (TypeScript 완성본)
//
// 사용법:
//   npx ts-node scripts/backfillFees.ts --teamId=xxx --months=12 --dryRun
//   npx ts-node scripts/backfillFees.ts --teamId=xxx --months=6 --dryRun=false
...
if (!admin.apps.length) {
  // serviceAccountKey.json 경로 확인 필요
  try {
...
async function backfillFees() {
  try {
    ...
    for (const teamDoc of teamsSnap.docs) {
      ...
      if (teamData.enableNewFeeSystem === false) {
    ...
    if (dryRun) {
      ...
      console.log(`   npx ts-node scripts/backfillFees.ts --teamId=${teamId || "ALL"} --months=${months} --dryRun=false`);
    }

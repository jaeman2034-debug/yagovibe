...
// .env 파일 목록 (나중 파일이 앞 값 덮어씀 — `.env.local`이 `.env`보다 우선)
...
function main() {
  ...
  console.log(`✅ .env 파일 발견: ${envPaths.map((p) => path.basename(p)).join(", ")}\n`);

  /** 뒤 파일이 앞 파일을 덮어씀 — Vite와 동일한 방향성 */
  const varMap = new Map();
  for (const envPath of envPaths) {
    const m = parseEnvFile(envPath);
    for (const [k, v] of m) {
      varMap.set(k, v);
    }
  }

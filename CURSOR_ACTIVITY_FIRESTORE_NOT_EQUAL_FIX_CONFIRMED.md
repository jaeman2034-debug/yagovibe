...
**확인 결과**: `where("type", "!=", "system")` 코드가 **2곳에 존재합니다.**
...
### 위치 1: loadInitial 함수 (101줄)

```typescript
} else {
  // 🔥 전체 탭: system 타입 제외
  activitiesConditions.push(where("type", "!=", "system"));  // ❌ 문제 코드
}
```

### 위치 2: loadMore 함수 (330줄)
...
Firestore에서는 `!=` 조건을 사용할 경우 **반드시 `orderBy`가 필요**합니다.

현재 코드:
```typescript
where("sport", "==", sport),
where("type", "!=", "system"),  // ❌ orderBy 없이 != 사용
orderBy("createdAt", "desc")
```

**문제**: `!=` 조건이 `orderBy`보다 먼저 나와서 Firestore 쿼리가 실패합니다.

---

## 수정 방법

`!=` 조건을 **Firestore 쿼리에서 완전히 제거**하고, 필터는 **클라이언트에서 처리**합니다.

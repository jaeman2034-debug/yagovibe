...
5. **이 API 키를 복사하세요!** (전체 키)

### Step 2: Firebase Console에서 API 키 확인
...
3. **브라우저 콘솔에 출력된 키와 Firebase Console의 키가 일치하는지 확인**
   - ✅ 일치하면 → Step 3으로
   - ❌ 다르면 → `.env.local` 파일 확인 필요!

### Step 3: Google Cloud Console에서 Browser key 찾기
...
3. **"API keys" 섹션에서 키 찾기**
   - 브라우저 콘솔에 출력된 **전체 API 키**와 일치하는 키 찾기
...
### Step 4: Browser key의 HTTP 리퍼러 제한 확인
...
3. **"API restrictions"** 섹션
   - **"Don't restrict key"** 선택되어 있는지 확인
   - 또는 **"Restrict key"** 선택 시:
     - ✅ Identity Toolkit API 포함되어 있는지 확인
     - ✅ Firebase Authentication API 포함되어 있는지 확인
...
`.env.local` 파일을 수정했다면 **반드시** 개발 서버를 재시작하세요:
...
- [ ] 브라우저 콘솔에서 실제 사용되는 API 키 확인 (전체 키)
- [ ] Firebase Console의 API Key와 일치하는지 확인
- [ ] Google Cloud Console에서 해당 Browser key 찾기
- [ ] Browser key의 HTTP 리퍼러 제한에 `localhost:5173/*` 추가 확인
